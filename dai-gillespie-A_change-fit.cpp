...
int main(){
  double C[MAX_ROD], D[MAX_ROD], t[MAX_ROD];
  double random1, random2, tau, alfa[5], beta, a[2][2],N, mediaC, mediaD, b;
  int i,j,k, l[MAX_ROD];
  fstream arquivo, arquivo2;
  arquivo.open("txt/dai-gillespie-A1_change-fit.txt",ios::out);
  arquivo2.open("txt/dai-gillespie-A2_change-fit.txt",ios::out);
  b = 1.2;
  beta=1.0/83341.0;
...
//  arquivo << t[0] << " " << C[0] << " " << 0 << " " << D[0] << " " << 0 << endl;
  ...
  for(k=0;k < 4; k++){
    ...
    for(j=0;j<MAX_ROD;j++){
      C[j]=pow(3,k)*800; D[j]=0;t[j]=0;l[j]=1; // Condicao inicial do sistema
    ...
    for (i=1; i<MAX_IT; i++){
      for(j=0;j<MAX_ROD;j++){
        ...
        random2 = uni(rand64);
        N = C[j]+D[j];
        alfa[1] = (beta*N + b*C[j]/N)*C[j]; //C morre
